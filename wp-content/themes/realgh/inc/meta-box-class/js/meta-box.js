...
function update_repeater_fields(){
    _metabox_fields.init('update');
}

//metabox fields object
var _metabox_fields = {
  oncefancySelect: false,
  init: function(state = null){
      ...
      return {
        ...
        hooks:function(){
          const btns = document.querySelectorAll(
            '.simplePanelimageUpload, .simplePanelfileUpload, .simplePanelimageUploadclear, .simplePanelfileUploadclear'
          )
          ...
          btns.forEach((btn) => btn.addEventListener('click', (event) => {
            ...
            if (btn.classList.contains('simplePanelfileUpload')) {
              inited.upload(btn,'file');
            }
            else {
              inited.upload(btn,'image');
            }

            if (
              !btn.classList.contains('simplePanelimageUploadclear') &&
              !btn.classList.contains('simplePanelfileUploadclear')
            ) {
              return
            }

            inited.set_fields(btn);
            $(inited.file_url).val("");
            $(inited.file_id).val("");

            if (btn.classList.contains('simplePanelimageUploadclear')) {
              inited.set_preview('image',false);
              inited.replaceImageUploadClass(btn);
            }
            else if (btn.classList.contains('simplePanelfileUploadclear')) {
              inited.set_preview('file',false);
              inited.replaceFileUploadClass(btn);
            }
          }))    
        },
