...
class AT_Meta_Box {
  ...
  public function __construct ( $meta_box ) {
	 ...
	 add_action( 'admin_print_styles', array( $this, 'is_load_scripts_styles' ) );
  ...
  /**
	...
	* @access public
	*/
  public function is_load_scripts_styles() {
	 ...
	 if ( isset($this->_meta_box['single']) && $this->_meta_box['single'] == true ) {
		global $post;

		if (!empty($post)) {
		  $pageTemplate = get_post_meta($post->ID, '_wp_page_template', true);

		  if($pageTemplate == $this->_meta_box['page_name'] ) {
			 $this->load_scripts_styles();
		  }
		}
	 }
	 else if (in_array($typenow,$this->_meta_box['pages']) && $this->is_edit_page()){
		$this->load_scripts_styles();
	 }
  }
  ...
  public function add($postType) {
	 ...
	 else if(in_array($postType, $this->_meta_box['pages'])){
		add_meta_box( $this->_meta_box['id'], $this->_meta_box['title'], array( $this, 'show' ),$postType, $this->_meta_box['context'], $this->_meta_box['priority'] );
	 }
  }
