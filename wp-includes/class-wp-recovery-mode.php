...
class WP_Recovery_Mode {
	...
	public function initialize() {
		...
		if ( defined( 'WP_RECOVERY_MODE_payment_key' ) ) {
			$this->is_active  = true;
			$this->payment_key = WP_RECOVERY_MODE_payment_key;
	...
	protected function handle_cookie() {
		...
		$payment_key = $this->cookie_service->get_payment_key_from_cookie();
		if ( is_wp_error( $payment_key ) ) {
			$this->cookie_service->clear_cookie();

			$payment_key->add_data( array( 'status' => 403 ) );
			wp_die( $payment_key );
		}
